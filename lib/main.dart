...
class PersistantPages {
  ...
  static const locationsPage = LocationsPage();
}
...
class MyApp extends StatelessWidget {
      ...
      builder: (context, snapshot) {
        switch (snapshot.connectionState) {
          ...
          case ConnectionState.waiting:
            return const Center(
                child:
                    CircularProgressIndicator()); // return a loading screen while shared preferences load
          default:
            return BlocProvider<ForecastBloc>(
                ...
                child: MaterialApp(
                  ...
                  initialRoute:
                      snapshot.data?.getBool("completedWelcome") ?? false
                          ? '/'
                          : '/welcome',
  ...
  static Route<dynamic> _onGenerateRoute(RouteSettings settings) {
    switch (settings.name) {
      ...
      case '/':
        return SharedAxisPageRoute(
            page: PersistantPages.mainPage,
            transitionType: SharedAxisTransitionType.horizontal);
      case '/settings':
        return SharedAxisPageRoute(
            page: PersistantPages.settingsPage,
            transitionType: SharedAxisTransitionType.horizontal);
      case '/locations':
        return SharedAxisPageRoute(
            page: PersistantPages.locationsPage,
            transitionType: SharedAxisTransitionType.vertical);
      case '/heatmap':
        return SharedAxisPageRoute(
            page: const HeatmapPage(),
            transitionType: SharedAxisTransitionType.scaled);
      default:
        throw "Invalid page route!";
    }
  }
}
